      ...
      {notifications.map((n) => (
        <div key={n._id} className="p-4 border rounded shadow-md flex justify-between items-start">
          ...
          <div className="flex flex-col gap-2 ml-4">
            <button onClick={() => onUpdate(n._id)} className="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded">Actualizar</button>
            <button onClick={() => onDelete(n._id)} className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded">Eliminar</button>
          </div>
        </div>
      ))}
    </div>
